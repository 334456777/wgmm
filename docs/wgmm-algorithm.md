...
5. 正向、负向事件数量**各自独立**达到 `PRUNE_THRESHOLD` 时，分别用 `prune_old_data()` 剪枝对应文件的低权重历史。
...
1. 批量计算所有扫描点得分，记录扫描均值。
2. 用梯度变号寻找原始局部峰（raw peak）。
3. 取**视野内首个得分高于扫描均值的局部峰**作为"下一次发布"预估——
   全局众数在多模态周期信号上系统性偏远，不是"下一到达时间"应有的数学对象。
4. 没有显著峰时，退回得分最高的 raw peak。
5. 完全没有局部峰时，使用全局最高分。

第二轮改进研究（ADR 007）确认：在首峰解码之后，序列依赖、非平稳衰减、
联合核、自适应带宽等候选机制均无系统性增益，当前结构即为该任务的合理形态。
