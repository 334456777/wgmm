...
└─────────────────────────────────────────────────────────────────────┘
    ...
    │       • 最小样本: 50 条 / 最短跨度: 168 小时（7天）
    │       • 构建小时级信号（每桶计事件数）
    │       • Wiener-Khinchin: FFT → |FFT|² → IFFT → 归一化自相关
    │       • 时域局部极大值筛选（阈值 0.02，范围 2天~90天）
    │       • 过滤已有4维周期（±20%）及整数倍/约数谐波
    │       • 按自相关强度降序返回最多3个新周期（秒）
    │
    ├─ 同步发现周期到配置
...
monitor.py:1204     - _discover_periods() - 自相关发现非日历周期
